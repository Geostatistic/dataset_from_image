...
c
c            integ = int(((value-cint(1))/(cint(2)-cint(1)))**0.6*255.)
            rtest = (r*r-((value-cmin)/(cmax-cmin)-0.0-xc)**2)
...
c
c            integ=int(((cmax-value)/(cmax-cint(4)))**0.6*255.)
            rtest = (r*r-(1.0-(value-cmin)/(cmax-cmin)-xc)**2)
            integ = int(max(0.0001,sqrt(rtest)+yc)*255/0.25)
            if(integ.gt.255) integ = 255
            if(integ.lt.0)   integ = 0
      end if
